...
import org.mule.extension.webcrawler.internal.crawler.SiteNode;
import org.mule.extension.webcrawler.internal.crawler.SitemapBuilder;
import org.mule.extension.webcrawler.internal.crawler.SitemapGenerator;
...
public class CrawlOperations {
  ...
  getSiteMap(
    ...
    try{
      ...
      PageLoadOptions pageLoadOptions = new PageLoadOptions(waitOnPageLoad, waitForXPath, extractShadowDom, shadowHostXPath);

      SiteNode root = SitemapBuilder.build(
          configuration,
          connection,
          url,
          targetPagesParameters.getMaxDepth(),
          targetPagesParameters.isRestrictToPath(),
          targetPagesParameters.getRegexUrlsFilterLogic(),
          targetPagesParameters.getRegexUrls(),
          pageLoadOptions);

      String sitemapXmlString = root != null ? SitemapGenerator.generateSitemapXml(root) : "";
